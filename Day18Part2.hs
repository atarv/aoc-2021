...
snailfishSum :: [SnailfishNum] -> SnailfishNum
snailfishSum = foldl1 add

toPairs :: [a] -> [[a]]
toPairs (x : xs) = fmap (\y -> y : [x]) xs ++ toPairs xs
toPairs [] = []
...
main = do
    ...
    let sortedByMagnitude = sortOn (Down . magnitude) snailfishNums
        highestMagnitudes = take (length sortedByMagnitude `div` 2) sortedByMagnitude
        pairs = toPairs highestMagnitudes
        permutedPairs = concatMap permutations pairs
        maxMagnitude = maximum $ fmap (magnitude . snailfishSum) permutedPairs
    print maxMagnitude
