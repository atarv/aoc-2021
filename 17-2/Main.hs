...
countVelocitiesThatHit :: Bounds -> Int
countVelocitiesThatHit bounds =
    let trajectories =
                -- Pruning these could make this program run faster
                fmap trajectory
                    $ [ ((x, y), (0, 0)) | x <- [1 .. 400], y <- [-107 .. 350] ]
        valid = filter (any (`isWithin` bounds) . fmap snd) trajectories
        validVelocities = fmap (fst . head) valid
    in length validVelocities
  where
...
main = do
    ...
    let highY = countVelocitiesThatHit bounds
    print highY
