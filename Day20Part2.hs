...
enhance :: Enhancement -> (Int, Image) -> (Int, Image)
enhance enhanceArr (iteration, sourceImage) =
    let
        (xs  , ys  ) = unzip $ Map.keys sourceImage
        (minX, maxX) = minMax xs
        (minY, maxY) = minMax ys
        getPixel pos = Map.findWithDefault
            (if even iteration then '.' else '#')
            pos
            sourceImage
    in
        ( iteration + 1
...
main = do
    ...
    let (_, enhanced) = iterate (enhance enhanceArr) (0, image) !! 50
